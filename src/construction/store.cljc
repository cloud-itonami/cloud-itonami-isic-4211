(ns construction.store
  ...
  `telecom.store`), this actor has FOUR actuation events acting on the
  SAME entity (a construction site): dispatching a disaster/severe-
  weather alert, authorizing work-resume, filing an accident report,
  filing a periodic inspection report -- each with its OWN history
  collection, sequence counter and dedicated double-actuation-guard
  boolean (`:alert-dispatched?` / `:work-resumed?` / `:accident-
  reported?` / `:periodic-report-filed?`, never a `:status` value),
  ...
  accident/periodic report was filed, on what jurisdictional basis,
  approved by whom' is always a query over an immutable log -- the
  audit trail a community trusting a construction operator needs, and
  the evidence an operator needs if a stop-work or resume decision is
  later disputed."
...
(defn demo-data
  "A small, self-contained site set covering all four actuation
  lifecycles plus the uncovered-jurisdiction / unresolved-hazard /
  no-injury failure modes, so the actor + tests run offline."
  []
  {:sites
...
(defn- site->tx [{:keys [id name jurisdiction wind-speed-actual rainfall-actual snowfall-actual
                         hazard-unresolved? injury-occurred? injury-description worker-contacts
                         alert-dispatched? alert-number
                         work-resumed? resume-number
                         accident-reported? accident-report-number
                         periodic-report-filed? periodic-report-number status]}]
  (cond-> {:site/id id}
...
(def ^:private site-pull
  [:site/id :site/name :site/jurisdiction
   ...
   :site/hazard-unresolved? :site/injury-occurred? :site/injury-description :site/worker-contacts-edn
   ...
   :site/periodic-report-filed? :site/periodic-report-number :site/status])

(defn- pull->site [m]
  (when (:site/id m)
    {:id (:site/id m) :name (:site/name m) :jurisdiction (:site/jurisdiction m)
